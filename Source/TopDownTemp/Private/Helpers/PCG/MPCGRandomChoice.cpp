...
UMPCGRandomChoiceSettings::UMPCGRandomChoiceSettings()
{
    bUseSeed = true;
}

FName UMPCGRandomChoiceSettings::AdditionalTaskName() const
{
    return NAME_None;
}

TArray<FPCGPinProperties> UMPCGRandomChoiceSettings::OutputPinProperties() const
{
...
FPCGElementPtr UMPCGRandomChoiceSettings::CreateElement() const
{
...
bool FPCGRandomChoiceElement::ExecuteInternal(FPCGContext* Context) const
{
    ...
    const UMPCGRandomChoiceSettings* Settings = Context->GetInputSettings<UMPCGRandomChoiceSettings>();
    check(Settings);
    ...
    for (const FPCGTaggedData& Input : Context->InputData.GetInputsByPin(PCGPinConstants::DefaultInputLabel))
    {
        ...
        {
            NumberOfElementsToKeep = FMath::Clamp(Settings->FixedNumber, 0, InPoints.Num() - 1);
        }
